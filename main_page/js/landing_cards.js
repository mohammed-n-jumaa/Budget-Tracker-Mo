...
fetch('./js/currencies.json')
    ...
    .then(data => {
        ...
        sortedCurrencies.sort((a, b) => order.indexOf(a.currencyCode) - order.indexOf(b.currencyCode));
        ...
        sortedCurrencies.forEach(currency => {
            ...
            const changeText = currency.change >= 0 ? `+${currency.change}%` : `${currency.change}%`;

            listItem.innerHTML = `
                <span>${currencyText}</span>
                <span class="${currency.change >= 0 ? 'text-success' : 'text-danger'}">${changeText}</span>
            `;
